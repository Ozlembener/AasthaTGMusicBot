...
from AasthaMusicBot.utils.decorators.admins import ActualAdminCB
from AasthaMusicBot.utils.decorators.language import language, languageCB
from AasthaMusicBot.utils.inline.settings import (
...
async def playmode_ans(client, CallbackQuery, _):
    ...
    if command == "CHANNELMODECHANGE":
        ...
        try:
            ...
            try:
                return await CallbackQuery.answer(_["setting_15"], show_alert=True)
            except:
                return
        ...
        chatmode = await get_chatmode(CallbackQuery.message.chat.id)
        if chatmode == "Group":
            await set_chatmode(CallbackQuery.message.chat.id, "Channel")
            Group = None
        else:
