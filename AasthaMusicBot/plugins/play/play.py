...
from AasthaMusicBot import Apple, Resso, SoundCloud, Spotify, Telegram, YouTube, app
from AasthaMusicBot.utils import seconds_to_min, time_to_seconds
from AasthaMusicBot.utils.database import get_chatmode, get_cmode, is_video_allowed
...
from AasthaMusicBot.utils.inline.play import (
    livestream_markup,
    playlist_markup,
    slider_markup,
    track_markup,
    ...
    if audio_telegram:
        ...
        if await YouTube.exists(url):
            ...
            if "track" in url:
                ...
                try:
                    details, plist_id, thumb = await Spotify.playlist(url)
                except Exception:
                    return await mystic.edit_text(_["play_3"])
                streamtype = "playlist"
                plist_type = "spotify"
            ...
            if duration_sec > DURATION_LIMIT:
                return await mystic.edit_text(
                    _["play_6"].format(DURATION_LIMIT_MIN, details["duration_min"])
                )
            try:
                await stream(
                    _,
...
async def play_playlists_command(client, CallbackQuery, _):
    ...
    if ptype == "spotify":
        try:
            result, spotify_id, thumb = await Spotify.playlist(videoid)
        except Exception:
            return await mystic.edit_text(_["play_3"])
        spotify = True
    if ptype == "apple":
